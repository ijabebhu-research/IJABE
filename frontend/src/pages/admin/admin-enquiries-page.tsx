...
export function AdminEnquiriesPage() {
  ...
  return <div className="space-y-6">
    ...
    {enquiries.length === 0 ? <Card><CardContent className="py-10 text-slate-700">No enquiries yet.</CardContent></Card> : enquiries.map((enquiry) => <details key={enquiry.id} className="group rounded-3xl border border-[#154734]/15 bg-white text-slate-950 shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 [&::-webkit-details-marker]:hidden"><div className="min-w-0"><p className="break-words font-semibold">{enquiry.subject}</p><p className="mt-1 break-words text-sm text-slate-600">{enquiry.name} · {new Date(enquiry.createdAt).toLocaleDateString()}</p></div><span className="shrink-0 rounded-full bg-[#e7f0e8] px-3 py-1 text-xs font-medium text-[#154734] group-open:hidden">View</span><span className="hidden shrink-0 rounded-full border border-[#154734]/25 px-3 py-1 text-xs font-medium text-[#154734] group-open:inline">Close</span></summary><Card className="rounded-t-none border-x-0 border-b-0 shadow-none">
